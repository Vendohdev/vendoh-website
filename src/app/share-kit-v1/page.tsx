...
function Card({ mode, cat, variant }: { mode: Mode; cat: string; variant: 0 | 1 }) {
  const previewSquare = `/share/v1?mode=${mode}&cat=${cat}&variant=${variant}&format=square`;
  const dl = (fmt: string, scale: number) =>
    `/share/v1?mode=${mode}&cat=${cat}&variant=${variant}&format=${fmt}&scale=${scale}`;
  ...
  return (
    <div className="rounded-2xl border border-border-light bg-elevated overflow-hidden">
      <div className="bg-surface p-3 flex items-center justify-center">
        ...
        <img src={previewSquare} alt={`${mode} ${cat} v${variant + 1}`} className="rounded-lg w-full h-auto shadow-sm" loading="lazy" />
      </div>
      <div className="px-3 py-2.5">
        <p className="text-xs font-semibold text-foreground mb-1.5">Variant {variant + 1}</p>
        <div className="flex flex-wrap gap-1.5">
          {[
            ["Square", dl("square", 3), file("square")],
            ["Story", dl("story", 3), file("story")],
            ["Link", dl("og", 2), file("og")],
          ].map(([label, href, fname]) => (
            <a
              key={label}
              href={href}
              download={fname}
              className="inline-flex items-center gap-1 rounded-full bg-vendoh-blue px-2.5 py-1 text-[11px] font-semibold text-white hover:bg-vendoh-blue-dark transition-colors"
            >
              <Download size={11} />
              {label}
            </a>
...
export default function ShareKitV1Page() {
      ...
      <section className="bg-vendoh-ink pt-32 pb-12 sm:pt-40 sm:pb-14">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-sm font-extrabold text-vendoh-orange-300 uppercase tracking-wider mb-4">
            Internal · Share kit V1
          </p>
          <h1 className="text-4xl sm:text-5xl font-bold text-white tracking-tight">
            Category Advert Carousels — V1
          </h1>
          <p className="mt-4 text-lg text-white/70 max-w-2xl mx-auto">
            13 categories × client &amp; vendor × 2 variants. Tap a format to
            download (Square 3240², Story 3240×5760, Link 2400×1260).
          </p>
