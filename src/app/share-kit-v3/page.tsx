...
function Card({ mode, cat, variant }: { mode: Mode; cat: string; variant: 0 | 1 }) {
  const preview = `/share/v3?mode=${mode}&cat=${cat}&variant=${variant}&format=square`;
  const dl = (fmt: string, scale: number) => `/share/v3?mode=${mode}&cat=${cat}&variant=${variant}&format=${fmt}&scale=${scale}`;
    ...
    <main className="bg-background">
      <section className="bg-vendoh-ink pt-32 pb-12 sm:pt-40 sm:pb-14">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-sm font-extrabold text-vendoh-orange-300 uppercase tracking-wider mb-4">Internal · Share kit V3</p>
          <h1 className="text-4xl sm:text-5xl font-bold text-white tracking-tight">Photo Advert Carousels — V3</h1>
